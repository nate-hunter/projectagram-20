...
export const GET_USER_EMAIL = gql`
    query getUserEmail($input: String!) {
        users(where: {_or: [{email: {_eq: $input}}, {phone_number: {_eq: $input}}, {username: {_eq: $input}}]}) {
            email
        }
    }
`
...
export const SUGGEST_USERS = gql`
    query suggestUsers($limit: Int!, $followerIds: [uuid!], $createdAt: timestamptz!) {
        users(limit: $limit, where: {_or: [{id: {_in: $followerIds}}, {created_at: {_gt: $createdAt}}]}) {
            id
            username
            name
            profile_image
...
// posts with the most likes and comments at the top, newest to oldest where the posts are not from the users they are following
